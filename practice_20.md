...
Создайте простое API для управления списком пользователей. Необходимо реализовать серверной приложение с подключением базы данных MongoDB (можно использовать любую не реляционную, документоориентированную базу данных) для хранения информации о пользователях.
...
В качестве ответа на задание необходимо прикрепить ссылку на репозиторий с реализованной практикой. Ссылка подгружается в соответствующий раздел СДО: Задания для самостоятельной работы -> Практические занятия 19-20.
